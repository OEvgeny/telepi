/*! telegram-markdown-v2 v0.0.5 (https://github.com/AndyRightNow/telegram-markdown-v2, rev 3531d36) and its bundled deps (remark/unified/mdast ecosystem). MIT — see LICENSE alongside this file. Bundled with esbuild; do not edit by hand. */
...
var require_html_comment_regex = __commonJS({
  "node_modules/html-comment-regex/index.js"(exports, module) {
    "use strict";
...
var Processor = class _Processor extends CallableInstance {
  ...
  copy() {
    ...
    while (++index2 < this.attachers.length) {
      const attacher2 = this.attachers[index2];
      destination.use(...attacher2);
    }
  ...
  freeze() {
    ...
    while (++this.freezeIndex < this.attachers.length) {
      const [attacher2, ...options] = this.attachers[this.freezeIndex];
      if (options[0] === false) {
        continue;
      }
      if (options[0] === true) {
      ...
      const transformer = attacher2.call(self, ...options);
...
function gfmTableToMarkdown(options) {
  ...
  function handleTable2(node2, _, state, info) {
...
var import_html_comment_regex = __toESM(require_html_comment_regex(), 1);
...
var transformer_default = removeComments;

// node_modules/remark-remove-comments/index.js
function attacher() {
  return transformer_default;
}
var remark_remove_comments_default = attacher;
...
function handleList(node2, parent, state, info) {
  const result = handle.list(node2, parent, state, info);
...
function handleListItem(node2, parent, state, info) {
  const result = handle.listItem(node2, parent, state, info);
...
function handleBlockquote(unsupportedTagsStrategy) {
  return function(node2, _parent, state, info) {
    const exit3 = state.enter("blockquote");
    const content3 = renderChildren(node2, state, info);
    exit3();
...
function handleHtml(unsupportedTagsStrategy) {
  return function(node2) {
    return processUnsupportedTags(node2.value, unsupportedTagsStrategy);
  };
}
function handleThematicBreak(unsupportedTagsStrategy) {
  return function() {
...
function handleHeading(node2, _parent, state, info) {
  const marker = "*";
  const exit3 = state.enter("headingAtx");
  const value = renderChildren(node2, state, {
    ...info,
    before: marker,
    after: marker
  });
...
function handleLink(node2, _parent, state, _info) {
  ...
  const text5 = renderChildren(node2, state, { ..._info, before: "[", after: "]" }) || (node2.title ? escapeSymbols(node2.title) : "");
  const isUrlEncoded = decodeURI(node2.url) !== node2.url;
  const url = isUrlEncoded ? node2.url : encodeURI(node2.url);
  exit3();
  if (!isURL(url)) return text5 || escapeSymbols(url);
  return text5 ? `[${text5}](${escapeSymbols(url, "link")})` : `[${escapeSymbols(url)}](${escapeSymbols(url, "link")})`;
}
function handleLinkReference(definitions) {
  return function(node2, _parent, state, _info) {
    ...
    const text5 = renderChildren(node2, state, { ..._info, before: "[", after: "]" }) || (definition3 ? definition3.title : null);
    exit3();
    if (!definition3 || !isURL(definition3.url)) return escapeSymbols(text5);
    return text5 ? `[${text5}](${escapeSymbols(definition3.url, "link")})` : `[${escapeSymbols(definition3.url)}](${escapeSymbols(definition3.url, "link")})`;
  };
}
function handleImage(node2, _parent, state, _info) {
  ...
  const url = node2.url;
  exit3();
  if (!isURL(url)) return escapeSymbols(text5) || escapeSymbols(url);
  return text5 ? `[${escapeSymbols(text5)}](${escapeSymbols(url, "link")})` : `[${escapeSymbols(url)}](${escapeSymbols(url, "link")})`;
}
function handleImageReference(definitions) {
  return function(node2, _parent, state, _info) {
    ...
    exit3();
    if (!definition3 || !isURL(definition3.url)) return escapeSymbols(text5);
    return text5 ? `[${escapeSymbols(text5)}](${escapeSymbols(definition3.url, "link")})` : `[${escapeSymbols(definition3.url)}](${escapeSymbols(definition3.url, "link")})`;
  };
}
...
function handleText(node2, _parent, state, _info) {
  ...
  exit3();
  return escapeSymbols(text5);
}
function handleInlineCode(node2, _parent, state, _info) {
  const exit3 = state.enter("paragraph");
  const value = escapeSymbols(node2.value, "code");
  exit3();
  return `\`${value}\``;
}
function handleCode(node2, _parent, state, _info) {
  ...
  const escapedContent = escapeSymbols(content3, "code");
  exit3();
  return wrap2(escapedContent, "```", "\n");
}
...
function createHandlers(definitions, unsupportedTagsStrategy) {
  return {
    ...
    thematicBreak: handleThematicBreak(unsupportedTagsStrategy)
  };
}
...
function convert2(markdown, unsupportedTagsStrategy = "keep") {
  ...
  let result = remark().use(remarkGfm).use(remark_remove_comments_default).use(collectDefinitions, definitions).use(removeDefinitions).use(remarkStringify, markdownOptions).processSync(processedMarkdown).toString().replace(/<!---->\n/gi, "");
